...
###
# 一键启动脚本（macOS .command 文件，可直接双击运行）
# 功能：
...
# 如果是通过双击运行，打开终端窗口并显示输出
if [ -t 0 ]; then
    # 在终端中运行
    :
else
    # 通过双击运行，需要打开终端窗口
    osascript -e 'tell application "Terminal" to do script "cd \"'"$ROOT_DIR"'\" && \"'"$0"'\" && exit"' > /dev/null 2>&1 &
    exit 0
fi
